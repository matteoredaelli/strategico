...
test.EvalItemData.e0 <- function() {
  ## NO DATA time series
  e0     <- EvalItemData(project.name=project.name, id=5, value="VALUE1", CONFIG=CONFIG)
  e0.bis <- EvalItemData(project.name=project.name, keys=c("ES","MOTO","DUCATI"), value="VALUE1", CONFIG=CONFIG)
  checkEquals(
              e0,
              e0.bis
              )
...
test.EvalItemData.e01 <- function() {
  ...
  e0     <- EvalItemData(project.name=project.name, id=1, value="VALUE2", CONFIG=CONFIG)
...
test.EvalItemData.e1 <- function() {
  e1     <- EvalItemData(project.name=project.name, keys=c("IT"), value="VALUE1", CONFIG=CONFIG)
  ...
  e11     <- EvalItemData(project.name=project.name, keys=c("IT"), value="VALUE2", CONFIG=CONFIG)
...
test.EvalItemData.e2 <- function() {
  e2     <- EvalItemData(project.name=project.name, keys=c("IT", "CAR"), value="VALUE1", CONFIG=CONFIG)
  checkEquals(
  ...
  e22     <- EvalItemData(project.name=project.name, keys=c("IT", "CAR"), value="VALUE2", CONFIG=CONFIG)
  checkEquals(
...
test.EvalItemData.e3 <- function() {
  ...
  e3 <- EvalItemData(project.name=project.name, keys=c("","CAR"), value="VALUE1", CONFIG=CONFIG)
